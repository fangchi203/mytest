...
<script type="text/javascript">
    ...
    function allClick() {
        //获得当前点击后全选按钮的状态
        var flag = $("#all").prop("checked");//返回的是一个布尔值  值随着复选框的变化而变化
        //将此状态赋值给下面五个复选框  $("input[name='ck']") 选择所有含有那么属性=ck的input元素
    ...
    function ckClick() {
        //得到下面五个复选框的个数
        var fiveLegth = $("input[name='ck']").length;
        //得到下面复选框被选中的个数
        var checkedLength = $("input[name='ck']:checked").length;
        //进行对比，改变全选复选框的状态
        if(fiveLegth == checkedLength){
            ...
            <c:when test="${info.list.size()!=0}">
                ...
                <div id="middle">
                    <table class="table table-bordered table-striped">
                        ...
                        <c:forEach items="${info.list}" var="p"><%--这里的info是pageinfo  里面有一个属性是list--%>
                            <tr>
                            ...
                            <nav aria-label="..." style="text-align:center;">
                                <ul class="pagination">
                                    ...
                                    <li>
                                        ...
                                        <a href="javascript:ajaxsplit(${info.nextPage})" aria-label="Next"><%--如果为最后一页，nextPage是0--%>
                                            <span aria-hidden="true">»</span></a>
                                    </li>
    ...
    function ajaxsplit(page) {
        ...
        $.ajax({
            ...
            success: function () {
				...
				这一点是通过 url 参数的特殊语法实现的。如果该字符串中包含一个或多个空格，
				紧接第一个空格的字符串则是决定所加载内容的 jQuery 选择器。   #table*/
                $("#table").load("http://localhost:8080/myWeb/admin/product.jsp #table");
            }
        });
